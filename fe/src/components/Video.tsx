...
export const Video = ({
    ...
    const handleTimeUpdate: React.ReactEventHandler<HTMLVideoElement> = useCallback((e) => {
        if (innerPlaying) {
            ...
            if (progressBarRef.current) {
                progressBarRef.current.style.width = (e.currentTarget.currentTime - leftIndex) / (rightIndex - leftIndex) * 100 + '%'
            }
        }
    ...
    >
        ...
        <div style={{
            ...
            color: '#fff',
            cursor: 'pointer'
        }} onClick={togglePlay}>{innerPlaying ? mouseEnter ? <PauseCircleFilled /> : "" : <PlayCircleFilled />}</div>
        <div style={{
            position: 'absolute',
            bottom: 0,
            height: '2px',
